...
// All users with section access can manage drive lines
...
export default function DriveLinesPage() {
  ...
  const [deletingLine, setDeletingLine] = useState<any>(null)
  ...
  function openDeleteDialog(line: any) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      var url = '/api/drive-lines'
      var method = 'POST'
      if (editingId) {
        url = '/api/drive-lines/' + editingId
        method = 'PUT'
      }
      ...
      if (res.ok) {
        toast.success(editingId
          ? (isRtl ? 'تم تعديل خط الحفر' : 'Drive line updated')
          : (isRtl ? 'تم إنشاء خط الحفر' : 'Drive line created')
        )
        setDialogOpen(false)
        setEditingId(null)
        ...
        toast.error(errData.message || (isRtl ? 'حدث خطأ' : 'Error'))
      }
    } catch {
  ...
  async function handleDelete() {
    ...
    try {
      var res = await authedFetch('/api/drive-lines/' + deletingLine.id, { method: 'DELETE' })
      if (res.ok) {
  ...
  return (
    <div className="space-y-4">
                          ...
                          {user && (
                            <div className="flex items-center gap-2 pt-1 border-t">
                              <Button
                                ...
                                onClick={function() { openEditDialog(line) }}
                              >
                              ...
                              <Button
                                ...
                                onClick={function() { openDeleteDialog(line) }}
                              >
      ...
      {/* Create / Edit Dialog */}
      <Dialog open={dialogOpen} onOpenChange={function(v) { setDialogOpen(v); if (!v) setEditingId(null) }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingId
                ? (isRtl ? 'تعديل بيانات خط الحفر' : 'Update drive line details')
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={function() { setDialogOpen(false); setEditingId(null) }}>
                {isRtl ? 'إلغاء' : 'Cancel'}
              </Button>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {isRtl
                ? ('هل أنت متأكد من حذف خط الحفر "' + (deletingLine?.lineNumber || '') + '"؟ لا يمكن التراجع عن هذا الإجراء.')
                : ('Are you sure you want to delete drive line "' + (deletingLine?.lineNumber || '') + '"? This action cannot be undone.')
              }
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={function() { setDeleteDialogOpen(false); setDeletingLine(null) }}>
              {isRtl ? 'إلغاء' : 'Cancel'}
            </Button>
