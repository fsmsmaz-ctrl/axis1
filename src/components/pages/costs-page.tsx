  ...
  async function fetchCosts() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        var errMsg = (errData && (errData.message || errData.details)) || ('HTTP ' + res.status)
        console.error('[CostsPage] API error:', res.status, errData)
        setFetchError(errMsg)
        setCosts([])
        setByCategory([])
        setTotal(0)
        setTotalRentalCost(0)
        setGrandTotal(0)
        setRentalAssets([])
        setLoading(false)
        return
      ...
      console.log('[CostsPage] API response: costs=' + (data.costs ? data.costs.length : 0) + ', total=' + (data.total || 0))
      setCosts(data.costs || [])
        ...
        <CardContent>
          {fetchError ? (
            <div className="rounded-xl border border-destructive/50 bg-destructive/5 p-4 text-center">
              <p className="text-sm font-medium text-destructive">{isRtl ? 'خطأ في جلب البيانات' : 'Error fetching data'}</p>
              <p className="text-xs text-muted-foreground mt-1 font-mono break-all">{fetchError}</p>
              <Button variant="outline" size="sm" className="mt-3" onClick={fetchCosts}>
